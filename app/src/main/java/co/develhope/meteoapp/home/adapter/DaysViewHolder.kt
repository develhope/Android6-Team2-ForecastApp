...
class DaysViewHolder(private val binding: ListHomeScreenBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(item: Days, onClick: (WeekItems) -> Unit) {
        ...
        binding.textDayOfMonthList.text =  item.forecast.date.format(DateTimeFormatter.ofPattern("dd/MM"))
        ...
        binding.imgSkyList.setWeatherIcon(getWeatherIconbasedonId(item.forecast.weatherIcon))
